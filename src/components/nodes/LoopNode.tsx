...
import type { WorkflowNodeData, LoopConfig } from '@/types';
...
function LoopNodeInner(props: NodeProps) {
  const { id } = props;
        ...
        <div className="flex items-center gap-2">
          ...
          {config.loopOver && (
            <span
              ...
              style={{ color: 'var(--color-text-muted)' }}
              title={config.loopOver}
            >
              over: {config.loopOver}
            </span>
          )}
        </div>
      </div>
